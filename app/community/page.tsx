...
import {
  MessageCircle, Users, Globe, MapPin, Briefcase, Calendar,
  ShoppingBag, Utensils, ChevronUp, Eye, Clock, Send, X,
  CheckCircle, Loader2, Pin, Trash2,
} from 'lucide-react'
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  MessageCircle, Users, Globe, MapPin, Briefcase, Calendar, ShoppingBag, Utensils,
}
...
export default function CommunityPage() {
  ...
  const handleDelete = async (topicId: string) => {
    ...
    try {
      const res = await fetch('/api/community/topics/' + topicId, { method: 'DELETE' })
      if (res.ok) {
            ...
            <div style={{ maxWidth: 900, margin: '0 auto' }}>
              ...
              {postStatus === 'success' ? (
                ...
                <div style={{ display: 'grid', gap: 10 }}>
                  <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Discussion title..." style={{ width: '100%', padding: '9px 12px', border: '0.5px solid var(--color-border-secondary)', borderRadius: 8, fontSize: 13, background: 'var(--color-background-secondary)', color: 'var(--color-text-primary)' }} />
                  <select value={newCategoryId} onChange={e => setNewCategoryId(e.target.value)} style={{ width: '100%', padding: '9px 12px', border: '0.5px solid var(--color-border-secondary)', borderRadius: 8, fontSize: 13, background: 'var(--color-background-secondary)', color: 'var(--color-text-primary)' }}>
                    <option value="">Select a category...</option>
                    {categories.map(c => (<option key={c.id} value={c.id}>{c.name}</option>))}
                  </select>
                  <textarea value={newContent} onChange={e => setNewContent(e.target.value)} placeholder="Share your thoughts, questions or experiences..." rows={4} style={{ width: '100%', padding: '9px 12px', border: '0.5px solid var(--color-border-secondary)', borderRadius: 8, fontSize: 13, background: 'var(--color-background-secondary)', color: 'var(--color-text-primary)', resize: 'none', fontFamily: 'var(--font-sans)' }} />
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={handlePost} disabled={postStatus === 'loading'} style={{ background: '#16a34a', color: 'white', fontWeight: 500, padding: '8px 18px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 13, display: 'inline-flex', alignItems: 'center', gap: 6, opacity: postStatus === 'loading' ? 0.7 : 1 }}>
          ...
          <aside>
            <div style={{ background: 'var(--color-background-primary)', border: '0.5px solid var(--color-border-tertiary)', borderRadius: 10, overflow: 'hidden', marginBottom: 12 }}>
              ...
              <div style={{ padding: '6px 0' }}>
                <button onClick={() => setActiveCategory('all')} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px', background: activeCategory === 'all' ? 'var(--color-background-secondary)' : 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: activeCategory === 'all' ? 'var(--color-text-primary)' : 'var(--color-text-secondary)', fontWeight: activeCategory === 'all' ? 500 : 400, textAlign: 'left' }}>
                ...
                {categories.map(cat => {
                  ...
                  return (
                    <button key={cat.id} onClick={() => setActiveCategory(cat.slug)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px', background: activeCategory === cat.slug ? 'var(--color-background-secondary)' : 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: activeCategory === cat.slug ? 'var(--color-text-primary)' : 'var(--color-text-secondary)', fontWeight: activeCategory === cat.slug ? 500 : 400, textAlign: 'left' }}>
          ...
          <div>
            {isLoading ? (
              ...
              <div style={{ background: 'var(--color-background-primary)', border: '0.5px solid var(--color-border-tertiary)', borderRadius: 10, padding: '3rem', textAlign: 'center' }}>
                ...
                <button onClick={() => session ? setShowNewTopic(true) : window.location.href = '/auth/login'} style={{ background: '#16a34a', color: 'white', fontWeight: 500, padding: '8px 18px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 13 }}>
                  Start a Discussion
                </button>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {topics.map(topic => (
                  <div key={topic.id} style={{ background: 'var(--color-background-primary)', border: topic.isPinned ? '0.5px solid var(--color-border-success)' : '0.5px solid var(--color-border-tertiary)', borderRadius: 10, padding: '12px 14px', display: 'grid', gridTemplateColumns: '38px 1fr', gap: 10 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                      <button onClick={() => handleVote(topic.id)} style={{ width: 32, height: 28, border: '0.5px solid', borderColor: votedTopics.has(topic.id) ? 'var(--color-border-success)' : 'var(--color-border-tertiary)', borderRadius: 6, background: votedTopics.has(topic.id) ? 'var(--color-background-success)' : 'none', cursor: session ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', color: votedTopics.has(topic.id) ? 'var(--color-text-success)' : 'var(--color-text-secondary)', fontSize: 12 }} title={session ? 'Upvote' : 'Sign in to vote'}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 4 }}>
                        ...
                        {isAdmin && (
                          <button
                            ...
                            style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, padding: '2px 8px', borderRadius: 6, border: '0.5px solid var(--color-border-danger)', background: 'var(--color-background-danger)', color: 'var(--color-text-danger)', cursor: 'pointer', opacity: deletingId === topic.id ? 0.5 : 1 }}
                          >
                            {deletingId === topic.id
                              ? <Loader2 style={{ width: 11, height: 11 }} />
